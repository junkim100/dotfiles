...
| 21 | `<leader>fy` / `<leader>fY` copy the buffer's path | The explorer yanks a path with `y`, but only for the entry under the cursor there; nothing copies the path of the buffer you are editing. `fy` gives the path relative to the project root, `fY` the absolute one. Both registers are set rather than just `+`, because the MOD 19 paste handler returns the unnamed register, so setting `+` alone would reach the laptop but leave `p` inside nvim pasting the previous yank. |
