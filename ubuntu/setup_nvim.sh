...
# -------------------------------------------------------------------- fd ----
# snacks prefers fd for file finding and falls back to rg, which is slower on a
# tree the size of solar-system. Ubuntu names the binary fdfind, same as it does
# batcat, so link it under the name the picker looks for.
if have fd; then
  ...
  echo "  NOTE: neither fd nor fdfind found. File finding falls back to ripgrep."
  echo "        apt-get install fd-find  (or install fd into ~/.local/bin)"
fi
